...
const GAME_INSTRUCTIONS: Record<string, { controls: string[]; howToPlay: string[] }> = {
  ...
  "frogger": { controls: ["Click plots to plant/harvest", "Buttons to buy/sell"], howToPlay: ["Plant seeds → grows in 5s → harvest", "Sell crops for gold (10g each)", "Buy seeds, fertilizer, extra fields", "Reach 200 total gold to win!"] },
  ...
  "hangman": { controls: ["Click letters on keyboard"], howToPlay: ["Guess the word before stick figure completes", "6 wrong guesses = game over", "Categories: tech, anime, games"] },
  "dotsboxes": { controls: ["Click between dots to draw lines"], howToPlay: ["Complete a box = your point + extra turn", "Most boxes wins", "Play vs AI"] },
  "numbermerge": { controls: ["Click two adjacent same-number tiles"], howToPlay: ["Merge matching numbers to double", "New tiles spawn after merges", "Game over when no merges possible"] },
  "checkers": { controls: ["Click piece → click destination"], howToPlay: ["Diagonal moves only", "Jump to capture", "Must jump if possible", "Reach opposite end for King"] },
  "solitaire": { controls: ["WASD Move", "E Eat berry", "C Craft menu", "F Fish"], howToPlay: ["Gather wood, stone, berries", "Craft tools: Axe, Campfire, Rod", "Build Raft (10 wood + 5 stone) to ESCAPE!", "Survive nights — enemies spawn in dark"] },
  "pinball": { controls: ["Z = Left flipper", "M = Right flipper"], howToPlay: ["Keep ball in play using flippers", "Hit bumpers for +10 points", "3 lives total", "Ball speeds up over time"] },
  "typingspeed": { controls: ["Type the shown word correctly"], howToPlay: ["60 seconds to type words", "Words get longer over time", "Tracks WPM and accuracy", "Fast and accurate = high score"] },
  "slidingpuzzle": { controls: ["Click tile adjacent to empty space"], howToPlay: ["Arrange tiles 1-15 in order", "Slide tiles into empty spot", "Fewer moves = better score", "Timer tracks your speed"] },
  "reactiontime": { controls: ["Click when screen turns green"], howToPlay: ["Wait for green signal", "Click as fast as possible", "Best of 5 attempts", "Tracks average reaction time"] },
  "memorysequence": { controls: ["Click highlighted squares in order"], howToPlay: ["Watch the sequence carefully", "Repeat by clicking same squares", "Sequence grows each round", "One mistake = game over"] },
  "tankbattle": { controls: ["WASD Move", "Space Shoot"], howToPlay: ["Destroy all 5 enemy tanks", "Use walls for cover", "Enemies shoot back", "3 lives total"] },
  "crossyroad": { controls: ["Space/tap to hop forward"], howToPlay: ["Cross roads — avoid cars", "Use logs on rivers", "Endless — score = distance", "Die if hit or in water"] },
  "bouncingball": { controls: ["Mouse to move platform"], howToPlay: ["Keep ball bouncing on platform", "Collect coins for bonus", "Speed increases over time", "Miss ball = game over"] },
  "coincollector": { controls: ["WASD to move"], howToPlay: ["Collect all coins on each level", "Avoid patrolling enemies", "10 levels total", "Timer affects scoring"] },
  "laserpuzzle": { controls: ["Click mirror to rotate 90°"], howToPlay: ["Guide laser from left to target", "Mirrors reflect the beam", "5 puzzle levels", "All must be solved to win"] },
  "pipeconnect": { controls: ["Click pipe to rotate 90°"], howToPlay: ["Connect water from source to drain", "Rotate pipe pieces to form path", "30 second timer per level", "Complete connection to win"] },
  "fallingsand": { controls: ["Click to place particles", "1=Sand 2=Water"], howToPlay: ["Sand falls with gravity", "Water flows and spreads", "Build landscapes freely", "Sandbox mode — no win/lose"] },
  "wordsearch": { controls: ["Click & drag to select words"], howToPlay: ["Find 5 hidden words in grid", "Words can be horizontal or vertical", "90 second timer", "Highlight found words"] },
  "dominoes": { controls: ["Click domino to play"], howToPlay: ["Match ends of played tiles", "Can't play? Draw from boneyard", "Empty your hand first to win", "Play vs AI opponent"] },
  "hexminesweeper": { controls: ["Click to reveal", "Right-click to flag"], howToPlay: ["Hexagonal grid with 6 neighbors", "Numbers show adjacent mines", "Flag all 10 mines to win", "Don't click a mine!"] },
  "battleship": { controls: ["Click enemy grid to attack"], howToPlay: ["Ships placed randomly on both sides", "Hit = red, Miss = gray", "Sink all 5 enemy ships to win", "AI attacks your fleet too"] },
  "escaperoom": { controls: ["Click items in correct order"], howToPlay: ["3 rooms to escape", "60 seconds per room", "Hints available", "Wrong order resets progress"] },
  "missilecommand": { controls: ["Click to launch interceptor"], howToPlay: ["Missiles fall toward your cities", "Click creates explosion at point", "Destroy missiles within range", "Protect all 4 cities"] },
  "stacktower": { controls: ["Click/Space to drop block"], howToPlay: ["Align moving block with stack", "Misaligned parts get trimmed", "Block gets smaller each miss", "Game over when too small"] },
  "cannondefense": { controls: ["Mouse to aim", "Click to fire"], howToPlay: ["Enemies approach from top", "Shoot them before they reach bottom", "3 lives", "Waves get faster"] },
  "ninjadash": { controls: ["Space Jump", "↓ Slide"], howToPlay: ["Auto-run endless runner", "Jump over walls", "Slide under beams", "Collect coins for bonus"] },
  "parkingmaster": { controls: ["↑ Forward", "←→ Turn"], howToPlay: ["Navigate car to parking spot", "5 levels with increasing difficulty", "Avoid walls and obstacles", "Timer affects score"] },
  "treasurehunter": { controls: ["WASD to move"], howToPlay: ["Collect all 5 treasures per level", "Avoid patrolling enemies", "Reach exit after collecting", "5 dungeon levels"] },
  "aliensurvivor": { controls: ["Mouse to move", "Auto-shoot nearest"], howToPlay: ["Aliens spawn from all edges", "Kill for XP, level up for upgrades", "Choose: Speed/Damage/Health", "Survive as long as possible"] },
  "goldminer": { controls: ["Click/Space to extend hook"], howToPlay: ["Hook swings left-right automatically", "Grab gold, gems, rocks", "Meet score target per level", "Big gold = slow reel, gems = fast"] },
};
