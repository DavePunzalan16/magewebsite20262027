...
export default function ProfilePage() {
  ...
  const toggleGameGenre = (genre: string) => {
    setSelectedGameGenres((prev) => prev.includes(genre) ? prev.filter((g) => g !== genre) : [...prev, genre]);
  };

  const handleSave = async () => {
    setSaving(true);
    // Will connect to Supabase profiles table later
  ...
  return (
    <div className="min-h-screen bg-background pb-20 pt-8">
      <div className="mx-auto max-w-[640px] px-6">
        {/* Back */}
        <Link href="/" className="mb-6 inline-flex items-center gap-1.5 font-body text-[13px] text-offwhite/60 transition-colors hover:text-primary">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          ...
          <div className="relative mb-8 overflow-hidden rounded-[16px] border border-dark-gray/30 bg-surface/30">
            ...
            <div className="relative px-6 pb-6">
              <div className="relative -mt-12 mb-4 inline-block">
                {avatarUrl ? (
                  <Image src={avatarUrl} alt="" width={80} height={80} className="rounded-full border-4 border-background object-cover" />
                ) : (
                ...
                <button className="absolute bottom-0 right-0 flex h-7 w-7 items-center justify-center rounded-full bg-primary text-black transition-transform hover:scale-110" aria-label="Change avatar">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="rounded-[12px] border border-dark-gray/30 bg-surface/20 p-5">
              <h2 className="mb-3 font-body text-[16px] font-semibold text-white">About Me</h2>
              <textarea
                value={bio}
            ...
            <div className="rounded-[12px] border border-dark-gray/30 bg-surface/20 p-5">
              ...
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="font-body text-[13px] font-medium text-offwhite/70">Favorite Anime</label>
                  <input type="text" value={favoriteAnime} onChange={(e) => setFavoriteAnime(e.target.value)}
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="font-body text-[13px] font-medium text-offwhite/70">Favorite Game</label>
                  <input type="text" value={favoriteGame} onChange={(e) => setFavoriteGame(e.target.value)}
                    className="rounded-[8px] border border-dark-gray/30 bg-background/50 px-4 py-2.5 font-body text-[14px] text-white placeholder:text-offwhite/30 focus:border-primary/40 focus:outline-none"
                    placeholder="e.g. Genshin Impact, Valorant" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="font-body text-[13px] font-medium text-offwhite/70">Favorite Character</label>
            ...
            {/* Anime genres */}
            <div className="rounded-[12px] border border-dark-gray/30 bg-surface/20 p-5">
              <h2 className="mb-3 font-body text-[16px] font-semibold text-white">Anime Genres</h2>
              <div className="flex flex-wrap gap-2">
                {genreOptions.map((g) => (
                  <button key={g} type="button" onClick={() => toggleGenre(g)}
                    className={`rounded-full border px-3 py-1.5 font-body text-[12px] font-medium transition-all ${selectedGenres.includes(g) ? "border-primary bg-primary/10 text-primary" : "border-dark-gray/40 text-offwhite/60 hover:border-primary/30"}`}>
                    {g}
                  </button>
                ))}
              </div>
            ...
            {/* Game genres */}
            <div className="rounded-[12px] border border-dark-gray/30 bg-surface/20 p-5">
              <h2 className="mb-3 font-body text-[16px] font-semibold text-white">Game Genres</h2>
              <div className="flex flex-wrap gap-2">
                {gameGenres.map((g) => (
                  <button key={g} type="button" onClick={() => toggleGameGenre(g)}
                    className={`rounded-full border px-3 py-1.5 font-body text-[12px] font-medium transition-all ${selectedGameGenres.includes(g) ? "border-primary bg-primary/10 text-primary" : "border-dark-gray/40 text-offwhite/60 hover:border-primary/30"}`}>
                    {g}
                  </button>
                ))}
              </div>
            ...
            <div className="flex items-center gap-3">
              <Button onClick={handleSave} disabled={saving} size="lg">
                {saving ? (
                  <><div className="mr-2 h-4 w-4 animate-spin rounded-full border-2 border-black/30 border-t-black" /> Saving...</>
                ) : (
                  <><Save className="mr-2 h-4 w-4" /> Save Profile</>
                )}
              </Button>
              {saved && (
                <motion.span className="font-body text-[13px] text-green-400" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  ✓ Saved successfully
                </motion.span>
              )}
            </div>
