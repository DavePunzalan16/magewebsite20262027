...
export default function SignUpPage() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background p-4">
        ...
        >
          ...
          <Link href="/auth/signin" className="mt-4 font-body text-[14px] font-semibold text-primary hover:opacity-80">
            Go to Sign In
          </Link>
        </motion.div>
      </div>
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      ...
      >
        <Link href="/" className="mb-6 inline-flex items-center gap-1 font-body text-[14px] text-offwhite hover:text-primary">
          <ArrowLeft className="h-4 w-4" /> Back to Home
        </Link>

        <div className="mb-8 flex flex-col items-center gap-4 text-center">
          <Image src={siteConfig.iconImage} alt="M.A.G.E. Guild" width={60} height={60} className="rounded-full" />
          <div>
            <h1 className="font-display text-[32px] text-white">Join The Guild</h1>
            <p className="font-body text-[14px] text-offwhite">Create your account</p>
          </div>
        </div>

        {error && (
          <motion.div className="mb-4 rounded-[8px] border border-red-500/30 bg-red-500/10 p-3" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <label htmlFor="signup-name" className="font-body text-[14px] font-medium text-offwhite">Full Name</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="signup-email" className="font-body text-[14px] font-medium text-offwhite">Email</label>
            <div className="relative">
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="signup-password" className="font-body text-[14px] font-medium text-offwhite">Password</label>
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-offwhite/50 hover:text-white" aria-label="Toggle password">
        ...
        <p className="mt-6 text-center font-body text-[14px] text-offwhite">
          Already have an account? <Link href="/auth/signin" className="font-semibold text-primary hover:opacity-80">Sign in</Link>
        </p>
      </motion.div>
    </div>
  );
