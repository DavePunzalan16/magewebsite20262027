...
import { Send, ImageIcon, Hash, Pin, Trash2, Eye, EyeOff, Edit3 } from "lucide-react";
...
export default function AdminPostsPage() {
  ...
  // PUBLISH — use direct Supabase insert (user is authenticated as admin)
  ...
  const handlePin = async (id: number, currentlyPinned: boolean) => {
    const supabase = createClient();
    await supabase.from("posts").update({ is_pinned: !currentlyPinned }).eq("id", id);
    setPosts((prev) => prev.map((p) => p.id === id ? { ...p, is_pinned: !currentlyPinned } : p));
  };

  const handleHide = async (id: number, currentlyHidden: boolean) => {
    const supabase = createClient();
    await supabase.from("posts").update({ is_hidden: !currentlyHidden }).eq("id", id);
    setPosts((prev) => prev.map((p) => p.id === id ? { ...p, is_hidden: !currentlyHidden } : p));
  };
  ...
  return (
    <div>
      ...
      <p className="mb-6 font-body text-[13px] text-offwhite/50">Create, edit, pin, hide, and delete posts. All persisted to Supabase.</p>
      ...
      <div className="mb-8 rounded-[12px] border border-dark-gray/30 bg-surface/20 p-5">
        ...
        {error && <p className="mt-2 font-body text-[12px] text-red-400">{error}</p>}
      </div>
      ...
      {loading ? (
        <div className="flex flex-col gap-2">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-16 animate-pulse rounded-[10px] bg-surface/30" />)}</div>
      ) : posts.length === 0 ? (
        <p className="font-body text-[13px] text-offwhite/40">No posts yet. Publish one above and it will appear here and in the Feed.</p>
      ) : (
            ...
            <div key={post.id} className={`rounded-[10px] border p-4 ${post.is_hidden ? "border-red-500/20 bg-red-500/5 opacity-60" : "border-dark-gray/30 bg-surface/20"}`}>
              {editingId === post.id ? (
                ...
                <>
                  ...
                  {post.image_url && (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={post.image_url} alt="" className="mb-2 h-16 w-auto rounded object-cover" />
                  )}
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-1">
                      ...
                      <button onClick={() => handlePin(post.id, post.is_pinned)} className={`rounded p-1 ${post.is_pinned ? "text-primary" : "text-offwhite/30 hover:text-primary"}`} title="Pin"><Pin className="h-3.5 w-3.5" /></button>
